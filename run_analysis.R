...
# Read all the .txt files according the UCI HAR framework into R.
...
# Store the result to the data frame "data".
data<-rbind(X_train, X_test)

# Using the features.txt file to assign descriptive variable names. 
names(data)[1:dim(data)[2]]<-make.names(features$V2, unique=TRUE)

# Extract only:the measurements on the "mean" and "std" (standard deviation)  for each measurement. 
data<-data[grepl("mean\\.\\.", names(data)) | grepl("std\\.\\.", names(data))]

# Appendind sublect's IDs to the data.
data<-cbind(data, rbind(subject_train, subject_test))
names(data)[dim(data)[2]]<-"Subject"

# Appending activity's IDs to the data.
...
# Converts "Activities" back to factors .
data$Activity<-as.factor(data$Activity) 

# Make an independent data set "tidy_data" &
# Melt the "data" data frame with respect to Subject and Activity & "tidy_data"is almost ready.
...
## In order to read in a table-format the data, type:
##temp<-read.table("tidy_data.txt", header=TRUE)
##View(temp)